...
const WorkerForm: React.FC<WorkerFormProps> = ({ worker, onSubmit }) => {
  const { register, handleSubmit, formState: { errors } } = useForm<Worker>({
    defaultValues: worker
  });

  const onSubmitHandler: SubmitHandler<Worker> = (data) => {
    onSubmit(data);
  };

  return (
    <form onSubmit={handleSubmit(onSubmitHandler)} className="space-y-4 mb-4" aria-label="Edit Worker Form">
      <input
        type="text"
